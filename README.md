...
起動直後の 3 秒は**前回のリセット理由**、続く 3 秒は**左右の接続状態**を
表示し、その後は消灯します（接続状態が変わると 3 秒だけ再表示）。

| タイミング | 色 | 意味 |
| --- | --- | --- |
| 起動 0〜3 秒 | 消灯 | 電源投入 / リセットボタン（正常） |
| | 青 | ウォッチドッグ（ハングを検出して再起動した） |
| | マゼンタ（点灯 / N 回点滅） | ファームウェア自身によるソフトリセット（N = 理由コード） |
| | 白 | CPU ロックアップ |
| 起動 3〜6 秒 / 変化時 | 緑 | もう片方と接続中 |
| | 赤 | もう片方と未接続 |

右半分はさらに、BLE プロファイルの切替 (`BT0`〜`BT4`) やホストの接続 / 切断の
たびに**プロファイル番号の回数だけ点滅**します (プロファイル 0 = 1 回):
